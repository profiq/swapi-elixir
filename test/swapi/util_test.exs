defmodule SWAPI.UtilTest do
  ...
  describe "paginate/2" do
    ...
    test "returns the set number of items when page size is specified" do
      assert {:ok, {[_], _}} = Util.paginate(Person, %{"page" => "1", "limit" => "1"})
    end

    test "returns correctly offset items for given page size" do
      assert {:ok, {[person], _}} = Util.paginate(Person, %{"page" => "2", "limit" => "1"})
      assert %{id: 2} = person
    end

    test "works with offset parameter" do
      assert {:ok, {[person | _], _}} = Util.paginate(Person, %{"offset" => "1"})
      assert %{id: 2} = person
    end

    test "works with offset and limit parameter" do
      assert {:ok, {[person], _}} = Util.paginate(Person, %{"offset" => "1", "limit" => "1"})
      assert %{id: 2} = person
    end

    test "returns correct next and previous offsets" do
      assert {:ok, {_, %{next: {:offset, 4}, previous: {:offset, 2}}}} =
               Util.paginate(Person, %{"offset" => "3", "limit" => "1"})
    end

    test "previous offset is nil for first item" do
      assert {:ok, {_, %{previous: nil}}} = Util.paginate(Person, %{"offset" => "0"})
    end

    test "last offset is nil for last item" do
      assert {:ok, {_, %{next: nil}}} = Util.paginate(Person, %{"offset" => "41"})
    end
    ...
    test "returns error when using pages and limit is not a number" do
      assert {:error, :bad_request} =
               Util.paginate(Person, %{"page" => "1", "limit" => "not_a_number"})
    end

    test "returns error when offset parameter is not a number" do
      assert {:error, :bad_request} = Util.paginate(Person, %{"offset" => "not_a_number"})
    end

    test "returns error when using offsets and limit parameter is not a number" do
      assert {:error, :bad_request} =
               Util.paginate(Person, %{"offset" => "1", "limit" => "not_a_number"})
    end

    test "returns error when both page and offset are provided" do
      assert {:error, :bad_request} = Util.paginate(Person, %{"page" => "1", "offset" => "1"})
    end
  end
end
